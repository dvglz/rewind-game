import { expect, test } from 'vitest';
import { hidesCompletedGameLock, shouldEnableHapticsDebug } from './testMode';

test('keeps the completed-game lock enabled by default', () => {
  expect(hidesCompletedGameLock('')).toBe(false);
  expect(hidesCompletedGameLock('?foo=bar')).toBe(false);
});

test('disables the completed-game lock only when test=1 is present', () => {
  expect(hidesCompletedGameLock('?test=1')).toBe(true);
  expect(hidesCompletedGameLock('?mode=results&test=1')).toBe(true);
  expect(hidesCompletedGameLock('?test=0')).toBe(false);
});

test('keeps the completed-game lock enabled in production even when test=1 is present', () => {
  const hidesLock = hidesCompletedGameLock as (search: string, isProduction: boolean) => boolean;

  expect(hidesLock('?test=1', true)).toBe(false);
});

test('keeps haptics debug disabled regardless of test mode or vibrate support', () => {
  expect(shouldEnableHapticsDebug('', false)).toBe(false);
  expect(shouldEnableHapticsDebug('?test=1', true)).toBe(false);
  expect(shouldEnableHapticsDebug('?test=1', false)).toBe(false);
});
